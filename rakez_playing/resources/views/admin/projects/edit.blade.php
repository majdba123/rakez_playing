        ...
        <div class="space-y-6">
            ...
            <div>
                ...
                <select id="type" name="type" required
                    ...
                    <option value="apartment" {{ old('type', $project->type) == 'apartment' ? 'selected' : '' }}>Apartment</option>
                    <option value="floor" {{ old('type', $project->type) == 'floor' ? 'selected' : '' }}>Floor</option>
                    <option value="unit" {{ old('type', $project->type) == 'unit' ? 'selected' : '' }}>Unit</option>
                </select>
                @error('type')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <select id="type_discount" name="type_discount" required
                    ...
                    <option value="percentage" {{ old('type_discount', $project->type_discount) == 'percentage' ? 'selected' : '' }}>Percentage (%)</option>
                    <option value="fixed" {{ old('type_discount', $project->type_discount) == 'fixed' ? 'selected' : '' }}>Fixed Amount</option>
                </select>
                @error('type_discount')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            ...
            <div class="bg-gray-50 rounded-lg p-4">
                <h4 class="font-semibold text-gray-800 mb-2">Project Information</h4>
                <div class="grid grid-cols-2 gap-2 text-sm">
                    <div class="text-gray-600">Created:</div>
                    <div class="text-gray-900">{{ $project->created_at->format('M d, Y H:i') }}</div>
                    
                    <div class="text-gray-600">Last Updated:</div>
                    <div class="text-gray-900">{{ $project->updated_at->format('M d, Y H:i') }}</div>
                </div>
            </div>
